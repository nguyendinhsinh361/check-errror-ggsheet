...
class Kind_D13_Service:
    ...
    def L13(self, answer, audio_question):
        arr_error = []
        if not helper.check_E3_written_by_japanese(answer):
            arr_error.append(73)
        if not helper.check_L3_has_max_four_answers(answer):
            arr_error.append(14)
        if not helper.check_L13_audio_question_contains_answers(answer, audio_question):
            arr_error.append(19)
        return arr_error
    ...
    def S13(self, corect_answer):
        arr_error = []
        if not helper.check_S13_type_number_and_like_number_audio(corect_answer):
            arr_error.append(32)
        return arr_error

    def T13(self, explain, text_question, audio_question, correct_answer, kana_answer, romanji_answer):
        arr_error = []
    ...
    def run(self):
        ...
        arr_error = [
            ...
            self.J13(kind_data["audio_question"]),
            self.L13(kind_data["answer"], kind_data["audio_question"]),
            self.M13(kind_data["kana_answer"], kind_data["answer"]),
            self.N13(kind_data["romanji_answer"], kind_data["kana_answer"]),
            self.S13(kind_data["corect_answer"]),
            self.T13(kind_data["explain"], kind_data["text_question"], kind_data["audio_question"],
